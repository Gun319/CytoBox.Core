...
namespace CytoBox.Core.Extensions
{
    public static class StringExtensions
    {
        ...
        private static readonly Regex EmailRegex = new(@"\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*", RegexOptions.IgnoreCase);

        private static readonly Regex MobileRegex = new("^1[0-9]{10}$");

        private static readonly Regex PhoneRegex = new(@"^(\d{3,4}-?)?\d{7,8}$");

        private static readonly Regex IpRegex = new(@"^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$");
        ...
        {
            return string.IsNullOrWhiteSpace(inputStr) ? inputStr : inputStr.Replace(oldStr, newStr);
        }
        ...
        {
            return string.IsNullOrWhiteSpace(inputStr) ? inputStr : Regex.Replace(inputStr, pattern, replacement);
        }
